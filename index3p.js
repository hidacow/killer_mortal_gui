...
class GlobalState {
    ...
    updateZoom() {
        if (screen.width > 900) {
            ...
            this.C_zoom = screen.width / 695
        } else {
            // reserve space for the phone status bar?
            this.C_zoom = (screen.height - 50) / 715
        }
...
class UI {
    ...
    reset() {
        ...
        for (let pidx=0; pidx<3; pidx++) {
            ...
            let seatWind = (3 + pidx - GS.gs.roundNum) % 3
    ...
    updateHandInfo() {
        for (let pidx=0; pidx<4; pidx++) {
            ...
            if (GS.gs.nukidoras[pidx] > 0){
                this.addHandTiles(pidx, 'call', ['4z'], false)
                let label = createParaElem(`${GS.gs.nukidoras[pidx]}`)

                this.calls[pidx].lastChild.appendChild(label)
            }
        }
    }
...
function relativeToHero(pidx) {
    let relIdx = pidx<3 ? (3 + GS.heroPidx - pidx) % 3 : pidx
    return relIdx
}
function relativeToHeroStr(pidx, English=0) {
    let relIdx = relativeToHero(pidx)
    let key = ['Hero', 'Kami', 'Shimo', 'Pot'][relIdx]
    if (English) {
        return key
    }
...
function onlyUpdateState() {
    ...
    for (let ply=0; ply <= GS.ply_counter; ply++) {
        ...
        if (event.type == 'tsumo') {
            ...
            GS.gs.handOver = true
        } else if (event.type == 'nukidora') {
            GS.gs.nukidoras[event.actor]++
            GS.gs.hands[event.actor].push(GS.gs.drawnTile[event.actor])
            GS.gs.drawnTile[event.actor] = null
            removeFromArray(GS.gs.hands[event.actor], event.pai)
            // GS.gs.calls[event.actor].push([event.pai])
        } else {
            console.log(event)
    ...
    for (let thisPidx=0; thisPidx<4; thisPidx++) {
        ...
        for (let event of GS.ge[GS.hand_counter]) {
            ...
            if ('danger' in event && event['danger'][thisPidx]) {
                for (let tenpaiPidx=0; tenpaiPidx<3; tenpaiPidx++) {
